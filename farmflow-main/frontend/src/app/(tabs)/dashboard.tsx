...
export default function DashboardScreen() {
  ...
  const handleToggleTask = useCallback(async (taskInput: Task | string) => {
    ...
    if (requiresChecklist) {
      Alert.alert(
        'Checklist Required',
        'This task must be completed from its checklist screen.',
        [
          { text: 'Cancel', style: 'cancel' },
          { text: 'Open Checklist', onPress: () => {
            const path = task.category === 'birds' ? '/checklist/birds' : task.category === 'fish' ? '/checklist/fish' : '/checklist/health';
            router.push(path);
          }},
        ]
      );
